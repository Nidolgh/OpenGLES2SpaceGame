...
// A single U16 character for keyboard input/display. We encode them as multi bytes UTF-8 when used in strings.
...
namespace ImGui
{
	...
	IMGUI_API bool DragFloat2(const char* label, float v[2], float v_speed = 1.0f, float v_min = 0.0f,
	                          float v_max = 0.0f, const char* format = "%.3f", float power = 1.0f);
	...
	// Widgets: HandleShipInput with Keyboard
	...
	IMGUI_API bool BeginPopupContextWindow(const char* str_id = NULL, ImGuiMouseButton mouse_button = 1,
...
enum ImGuiColorEditFlags_
{
	...
	// [HandleShipInput]      // ColorEdit, ColorPicker: input and output data in RGB format.
	ImGuiColorEditFlags_InputHSV = 1 << 28,
	// [HandleShipInput]      // ColorEdit, ColorPicker: input and output data in HSV format.
...
struct ImGuiIO
{
	...
	// = 60.0f          // [BETA] Compact window memory usage when unused. Set to -1.0f to disable.
	...
	// Optional: Notify OS HandleShipInput Method Editor of the screen position of your cursor for text input position (e.g. when using Japanese/Chinese IME on Windows)
	...
	//------------------------------------------------------------------
	// HandleShipInput - Fill before calling NewFrame()
	//------------------------------------------------------------------
...
{
	unsigned int ID;
	// HandleShipInput    // User ID. Use < 0x110000 to map into a font glyph, >= 0x110000 for other/internal/custom texture data.
	unsigned short Width, Height; // HandleShipInput    // Desired rectangle dimension
	unsigned short X, Y; // Output   // Packed position in Atlas
	float GlyphAdvanceX; // HandleShipInput    // For custom font glyphs only (ID < 0x110000): glyph xadvance
	ImVec2 GlyphOffset; // HandleShipInput    // For custom font glyphs only (ID < 0x110000): glyph display offset
	ImFont* Font; // HandleShipInput    // For custom font glyphs only (ID < 0x110000): target font
