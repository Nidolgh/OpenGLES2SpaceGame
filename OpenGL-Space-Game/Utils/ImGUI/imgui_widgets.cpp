	...
	if (g.ActiveId == id)
	{
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse)
		{
			...
			else
			{
				...
				{
					bool is_double_click_release = (flags & ImGuiButtonFlags_PressedOnDoubleClick) && g
					                                                                                  .IO.
					                                                                                  MouseDownWasDoubleClick
						[0];
					bool is_repeating_already = (flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDurationPrev[0] >= g
					                                                                                                  .
					                                                                                                  IO
					                                                                                                  .KeyRepeatDelay;
					// Repeat mode trumps <on release>
                          ...
                          const char* format, float power, ImGuiDragFlags flags)
{
	...
	bool is_drag_direction_change_with_power = is_power && ((adjust_delta < 0 && g.DragCurrentAccum > 0) || (
	...
	if (is_power)
	{
		...
		FLOATTYPE v_old_norm_curved = ImPow((FLOATTYPE)(v_cur - v_min) / (FLOATTYPE)(v_max - v_min),
	...
	if (is_power)
	{
		FLOATTYPE v_cur_norm_curved = ImPow((FLOATTYPE)(v_cur - v_min) / (FLOATTYPE)(v_max - v_min),
		                                    (FLOATTYPE)1.0f / power);
                       ...
                       const void* p_max, const char* format, float power)
{
	...
	// Drag behavior
	const bool value_changed = DragBehavior(id, data_type, p_data, v_speed, p_min, p_max, format, power,
	                                        ImGuiDragFlags_None);
	if (value_changed)
		MarkItemEdited(id);
                            ...
                            ImRect* out_grab_bb)
{
	...
	{
		// Different sign
		const FLOATTYPE linear_dist_min_to_0 = ImPow(v_min >= 0 ? (FLOATTYPE)v_min : -(FLOATTYPE)v_min,
		                                             (FLOATTYPE)1.0f / power);
		const FLOATTYPE linear_dist_max_to_0 = ImPow(v_max >= 0 ? (FLOATTYPE)v_max : -(FLOATTYPE)v_max,
		                                             (FLOATTYPE)1.0f / power);
	...
	if (g.ActiveId == id)
	{
		...
		else if (g.ActiveIdSource == ImGuiInputSource_Nav)
		{
			...
			{
				clicked_t = SliderCalcRatioFromValueT<TYPE, FLOATTYPE>(data_type, *v, v_min, v_max, power,
				                                                       linear_zero_pos);
                         ...
                         const char* format, float power)
{
	...
	// Slider behavior
	ImRect grab_bb;
	const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, power,
...
bool ImGui::TempInputTextScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data,
                                const char* format)
{
	ImGuiContext& g = *GImGui;
...
// Note: p_data, p_step, p_step_fast are _pointers_ to a memory address holding the data. For an HandleShipInput widget, p_step and p_step_fast are optional.
                        ...
                        ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	const bool focus_requested_by_code = focus_requested && (g.FocusRequestCurrWindow == window && g.
		FocusRequestCurrCounterAll == window->DC.FocusCounterAll);
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		const bool is_osx = io.ConfigMacOSXBehaviors;
		const bool is_shortcut_key = (is_osx ? (io.KeySuper && !io.KeyCtrl) : (io.KeyCtrl && !io.KeySuper)) && !io.
			KeyAlt && !io.KeyShift; // OS X style: Shortcuts using Cmd/Super instead of Ctrl
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
		...
		ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		for (int n = 0; n < components; n++)
		{
			...
			{
				value_changed |= DragFloat(ids[n], &f[n], 1.0f / 255.0f, disable_hue_edit ? +FLT_MAX : 0.0f,
				                           disable_hue_edit ? -FLT_MAX : hdr ? 0.0f : 1.0f,
	...
	{
		// RGB Hexadecimal HandleShipInput
		char buf[64];
		if (alpha)
	...
	if (!is_leaf)
	{
		...
		if (pressed)
		{
			...
			if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
				toggled |= (g.IO.MousePos.x >= arrow_hit_x1 && g.IO.MousePos.x < arrow_hit_x2) && (!g.
...
bool ImGui::CollapsingHeader(const char* label, bool* p_open, ImGuiTreeNodeFlags flags)
{
	...
	flags |= ImGuiTreeNodeFlags_CollapsingHeader | (p_open
...
bool ImGui::BeginMenuBar()
{
	...
	ImRect clip_rect(IM_ROUND(bar_rect.Min.x), IM_ROUND(bar_rect.Min.y + window->WindowBorderSize),
	                 IM_ROUND(ImMax(bar_rect.Min.x, bar_rect.Max.x - window->WindowRounding)),
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
	...
	else
	{
		...
		float w = window->MenuColumns.DeclColumns(label_size.x, 0.0f, IM_FLOOR(g.FontSize * 1.20f));
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
	...
	else
	{
		...
		float w = window->MenuColumns.DeclColumns(label_size.x, shortcut_size.x, IM_FLOOR(g.FontSize * 1.20f));
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
	...
	if ((flags & ImGuiTabBarFlags_Reorderable) && !(tab_bar->Flags & ImGuiTabBarFlags_Reorderable) && tab_bar->Tabs.Size
		> 1 && tab_bar->PrevFrameVisible != -1)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
	...
	tab_bar->OffsetNextTab = offset_x;
	...
	const bool scrolling_buttons = (tab_bar->OffsetMax > tab_bar->BarRect.GetWidth() && tab_bar->Tabs.Size > 1) && !(
		tab_bar->Flags & ImGuiTabBarFlags_NoTabListScrollingButtons) && (tab_bar->Flags &
